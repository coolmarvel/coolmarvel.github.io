export interface Project {
  slug: string;
  name: string;
  oneLiner: string;
  period: string;
  company: string;
  domain: "의료" | "커머스" | "블록체인" | "AI" | "데스크톱" | "웹 서비스";
  featured?: boolean;
  description: string[];
  stack: string[];
  highlights: string[];
}

export const projects: Project[] = [
  {
    slug: "sh-messenger",
    name: "sh-messenger — 직접 만든 데스크톱 메신저 (Windows · macOS)",
    oneLiner: "가입·친구·실시간 대화·첨부·자동 업데이트까지 직접 만든 개인 메신저 — 대화 내용은 서버가 아니라 각자 PC에 남고, 운영 서버는 Lightsail 1대에서 돕니다",
    period: "2026.09 ~",
    company: "개인 프로젝트",
    domain: "데스크톱",
    featured: true,
    description: [
      "재직 중인 병원에서 쓰려고 만들었던 사내 메신저에서 회사 색(그룹웨어 계정 연동·조직도·이름·도메인)을 전부 걷어내고, 이메일로 가입하면 누구나 쓸 수 있는 개인 메신저로 다시 만들었습니다. 서버(NestJS 채팅·업로드·푸시·업데이트)는 이식해 살리고 Electron 클라이언트의 UI/UX는 처음부터 새로 짰습니다. 창 구조는 카카오톡 PC를 따랐습니다 — 좁은 메인 창(420×760) 하나에 친구·대화·검색·설정을 넣고, 대화는 방마다 별도 창으로 엽니다.",
      "설계에서 가장 오래 고민한 것은 \"내 대화가 남의 서버에 쌓이지 않게\"입니다. 메시지 본문과 첨부의 원본은 각자 PC(node:sqlite + 파일)에 두고, 서버는 전달과 다른 PC 동기화를 위해 보관 기간(기본 14일, 못 받은 사람이 있으면 최대 90일)만 갖고 있다가 지웁니다. 로그인은 카카오톡 PC처럼 한 번에 한 대만 허용합니다. 현재는 PC 전용입니다 — 서버는 모바일 푸시(FCM)까지 갖췄지만 모바일 클라이언트는 이번 범위 밖이라 아직 만들지 않았습니다.",
    ],
    stack: [
      "TypeScript",
      "Electron 38",
      "React 19",
      "electron-vite",
      "NestJS 11",
      "Prisma 6",
      "PostgreSQL 16",
      "Redis 7",
      "Socket.IO",
      "node:sqlite",
      "Docker",
      "Caddy",
      "Playwright",
      "Vitest",
    ],
    highlights: [
      "카카오톡 PC식 창 구조(ADR-0003) — 메인 창 420×760 + 대화마다 별도 창, 프레임 없는 자체 타이틀바, 창 위치·크기 기억, 자석 스냅, 트레이·작업 표시줄 안 읽음 배지",
      "전달형 저장(ADR-0006·0009) — 대화 내용·첨부 원본은 각자 PC(node:sqlite), 서버는 보관 기간 14일(미수신 시 최대 90일) 뒤 삭제. 새 PC로 로그인하면 그 기간만큼 내려받습니다",
      "PC 1대 로그인 — 토큰에 세션 id를 실어 다른 PC에서 로그인하면 되묻고(409 SESSION_ACTIVE) 옛 세션을 즉시 밀어냅니다(401·소켓 session:replaced)",
      "실시간 — Socket.IO + Redis 어댑터로 메시지·읽음·입력 중·접속 상태·반응을 동기화하고, 서버가 거절·종료한 소켓은 토큰을 회전해 직접 재접속합니다",
      "첨부 — presign 업로드(진행률)·640px WebP 썸네일·사진 묶음 격자·동영상 인라인·받은 파일 폴더(원래 이름)·대화 서랍(사진/파일/링크)·자체 사진 뷰어 창",
      "자동 업데이트 서버를 직접 운영 — exe·blockmap·latest.yml을 sha512로 검증해 게시하고, 마이너 승격은 필수·패치는 선택 업데이트로 내려보냅니다",
      "2026-09-17 킥오프 후 사흘 만에 v0.1.1 → v0.3.7 15 릴리스 · ADR 9건 · 테스트 서버 332 + 데스크톱 38 + 업데이트 서버 27 + Playwright 브라우저 38 + 실제 Electron 앱 E2E 5",
    ],
  },
  {
    slug: "sh-web-editor",
    name: "sh-web-editor — 스크립트 한 줄로 붙는 WYSIWYG 웹에디터",
    oneLiner: "Tiptap 위에 툴바·대화상자·아이콘을 전부 직접 만든 업무용 HTML 에디터 — 표 편집·양식·파일 업로더까지 한 번들, 라이브 데모 운영 중",
    period: "2026.08 ~",
    company: "개인 프로젝트",
    domain: "웹 서비스",
    featured: true,
    description: [
      "Asana·Jira 같은 협업 도구를 쓰면서 무료로 쓸 수 있는 텍스트 에디터가 UI/UX·기능·성능 모두 아쉬웠고, 그룹웨어에서는 진단서·소견서 같은 제증명 서류를 정해진 양식 위에 채워 넣는 일이 잦았습니다. 그래서 어떤 웹 페이지에든 <script> 한 줄로 붙고, 국내 업무 시스템에 익숙한 메뉴바·툴바·대화상자를 갖춘 WYSIWYG HTML 에디터를 직접 만들었습니다.",
      "Tiptap(ProseMirror) 엔진 위에 프레임워크 없이 TypeScript + CSS로 메뉴 8개·툴바 2줄(버튼 67)·대화상자 34종·SVG 아이콘 64종을 전부 자체 제작했고, 표 편집(병합·분할·셀 테두리·계산식), 양식(HTML 그대로 저장·불러오기), 파일 업로더(청크·이어올리기·zip)를 한 번들에 담았습니다. 양식 서버는 Java 21 + Spring Boot 4 + PostgreSQL 기준 구현을 두고, 공개 데모는 서버 없이 정적 파일로 배포했습니다.",
    ],
    stack: [
      "TypeScript",
      "Tiptap 3",
      "ProseMirror",
      "Vite 8",
      "DOMPurify",
      "Vitest",
      "Playwright",
      "Java 21",
      "Spring Boot 4",
      "PostgreSQL 17",
      "Flyway",
      "Docker",
      "Caddy",
      "oh-my-design",
    ],
    highlights: [
      "프레임워크 무관 단일 번들(IIFE + ESM + CSS + d.ts) — textarea에 붙이면 값이 자동 동기화돼 기존 폼 제출이 그대로 동작, React·Vue·JSP 어디든 삽입",
      "표 편집 — 업무 문서용 표 HTML 생성·보존, 우클릭 표 메뉴 15항목, 표 대화상자 7종, 표 손잡이·8핸들·끌어 옮기기, 계산식 10종",
      "양식(템플릿) — 결재란·회의록·휴가신청서 같은 서식을 HTML 그대로 저장·불러오기, 서버 주소 하나로 프로젝트별 양식 세트 교체(고정 API 계약)",
      "관리자(폼) 모드 — 입력 요소 8종·누름틀·표/셀 잠금으로 양식 구조를 잠그고, 사용자 모드는 값만 채움",
      "파일 업로더 — 청크 1MB + 이어올리기, 폴더 업로드, 전송 창(속도·남은 시간), 서버 없이도 브라우저에서 zip으로 묶어 다운로드",
      "XSS 정제(DOMPurify)·웹 접근성 검증·금칙어/개인정보 탐지·한/영 변환·계산기·맞춤법 연동·자동 저장·스킨 13종·모바일 모드",
      "2026-09-07~09 사흘 동안 v0.1.1 → v1.0.5 42회 릴리스 — 사용자 스크린샷 피드백 15회차를 상용 에디터 실물 픽셀 실측과 대조하며 반영",
    ],
  },
  {
    slug: "remote-assist",
    name: "remote-assist — 접속 코드 원격 지원 (Windows · macOS)",
    oneLiner: "코드 하나로 붙고 사용자가 수락해야 열리는 원격 지원 앱 — 중계 서버 직접 운영, 4일간 15 릴리스",
    period: "2026.09 ~",
    company: "개인 프로젝트",
    domain: "데스크톱",
    featured: true,
    description: [
      "도움이 필요한 사람(Agent)과 도움을 주는 사람(Console)을 접속 코드 하나로 잇는 데스크톱 원격 지원 앱입니다. 지원자가 코드를 입력하면 사용자 PC에 수락 요청이 뜨고, 사용자가 수락해야만 화면 공유와 마우스·키보드 제어가 시작됩니다. 같은 망이든 인터넷 건너편이든 붙는 방법은 같고(직접 운영하는 중계 서버 경유), 양쪽 누구든 한 번 클릭으로 끊습니다.",
      "C#/.NET 8 한 언어로 Windows(WPF)·macOS(Avalonia) 클라이언트 4종과 ASP.NET Core 중계 서버, 공용 Core를 만들었습니다. DXGI 캡처·H.264 하드웨어 인코딩·회선 적응 5단계·같은 망 직결까지 갖췄고, 플랫폼 전용 경로마다 자동 폴백(GDI·JPEG·중계)을 뒀습니다. 상용 원격 도구의 라이선스 비용과 '남의 서버를 경유해 화면이 밖으로 나가는' 부담을 없애는 것이 동기였습니다.",
    ],
    stack: [
      "C# 12",
      ".NET 8",
      "WPF",
      "Avalonia UI 11",
      "ASP.NET Core 8",
      "WebSocket",
      "SkiaSharp",
      "DXGI",
      "Media Foundation",
      "Docker",
      "Caddy",
      "Inno Setup",
      "xUnit",
    ],
    highlights: [
      "수락 필수 · 1회용 6자리 코드(10분 만료) · TLS(Caddy) — 중계 서버는 바이트만 통과시키고 화면을 해석하지 않음",
      "DXGI dirty rect 캡처 + JPEG 타일/H.264 하이브리드 + 회선 적응 5단계(품질·fps·해상도) + 커서 분리 60Hz",
      "화면/제어 2채널 WebSocket — 같은 망이면 세션마다 RSA 자체 서명 TLS + 지문 고정으로 직결(서버 트래픽 0)",
      "세션 로직을 Core로 이관하고 OS 의존은 IAgentPlatform 뒤로 — 가짜 플랫폼으로 실제 중계 위 종단 테스트(xUnit 217)",
      "Windows↔macOS 세션에서 Ctrl↔⌘ 자동 치환, hello caps 협상으로 디코더 없는 상대에겐 JPEG만 전송",
      "킥오프부터 4일간 v0.1.0 → v0.1.15 — Windows 인스톨러 2종 + macOS dmg 2종 + Docker 서버 배포",
    ],
  },
  {
    slug: "pdf-editor-live",
    name: "PDF Editor Live — 브라우저 PDF 편집 웹 서비스",
    oneLiner: "PDF가 서버로 가지 않는 브라우저 편집기를 회원제·구독 웹 서비스로 — AWS Lightsail 운영 중",
    period: "2026.08 ~",
    company: "개인 프로젝트",
    domain: "웹 서비스",
    featured: true,
    description: [
      "직접 만든 데스크톱 PDF 편집기(Electron)를 웹 서비스로 옮겨 https://pdf-editor.coolmarvel.com 에 배포했습니다. pdf.js 렌더와 pdf-lib 조립이 전부 브라우저에서 끝나고, 서버는 계정·구독·내보내기 로그만 다루는 것이 설계의 한 가지 약속입니다.",
      "Vite+React SPA / Fastify+Drizzle API / PostgreSQL 16 / Caddy 리버스 프록시를 npm 워크스페이스 모노레포로 묶고, GitHub Actions가 이미지를 빌드해 Lightsail 2GB 서버에 배포합니다. 이메일 가입과 Google·Kakao·Naver 소셜 로그인, 환영 체험 14일 구독 모델, 검색엔진 프리렌더까지 4일 만에 운영 상태로 올렸습니다.",
    ],
    stack: [
      "Vite",
      "React 18",
      "TypeScript",
      "MUI 6",
      "pdf.js",
      "pdf-lib",
      "Node 22",
      "Fastify 5",
      "Drizzle ORM",
      "PostgreSQL 16",
      "Caddy",
      "Docker",
      "GitHub Actions",
      "AWS Lightsail",
      "OAuth 2.0",
      "Playwright",
    ],
    highlights: [
      "데스크톱 에디터 코드를 Platform 인터페이스로 추상화 — Electron과 웹이 같은 에디터를 공유(양방향 동기화)",
      "Google·Kakao·Naver 소셜 로그인 — 서버측 authorization code 교환, 서명된 state 쿠키(CSRF), 같은 이메일 자동 연동",
      "구독 전용 과금 — 환영 체험 14일, 편집은 무료·내보내기 시점에만 402 게이트, 미소비 토큰 자동 환불",
      "Lightsail 2GB 1대에 Caddy(TLS 자동)·공용 Postgres·API 컨테이너를 메모리 예산으로 운영(API RSS 약 85MB)",
      "SPA 라우트별 프리렌더 + sitemap + OG/JSON-LD — Google·네이버·다음 검색 등록",
      "API 통합 테스트 28건 + Playwright E2E 46건(데스크톱·모바일·좁은 폭 3프로젝트)",
    ],
  },
  {
    slug: "pdf-editor",
    name: "PDF Editor — 오프라인 데스크톱 편집기",
    oneLiner: "문서가 밖으로 나가지 않는 로컬 PDF 편집기 (Electron)",
    period: "2026.07 ~",
    company: "개인 프로젝트",
    domain: "데스크톱",
    description: [
      "페이지 관리, 텍스트 추가/수정, 그리기, 도형, 스탬프, 서명, 주석, 링크까지 지원하는 오프라인 데스크톱 PDF 편집기입니다. 모든 좌표를 0..1로 정규화한 객체 모델로 라이브 오버레이와 내보내기가 단일 렌더러를 공유해 \"화면=결과물 1:1\"을 보장합니다.",
      "스크린샷 피드백 루프, 세션 부팅 프로토콜, typecheck·test·build 통과 시에만 인스톨러를 굽는 자동 검증 파이프라인 등 AI 협업 하네스를 데스크톱 앱 개발에 적용했습니다.",
    ],
    stack: [
      "Electron 43",
      "React 18",
      "TypeScript",
      "MUI 6",
      "pdf.js",
      "pdf-lib",
      "zustand",
      "lucide",
      "Playwright",
    ],
    highlights: [
      "참조 기반 페이지 연산 — 원본 PDF 바이트 불변 유지",
      "저장 시 오버레이 평탄화 + 링크/노트는 실제 PDF 주석으로 삽입",
      "Windows(NSIS)·macOS(DMG, Apple Silicon) 멀티플랫폼 인스톨러 배포",
      "v1.5→v1.7.0 — 문서 검색(Ctrl+F)·페이지 관리 모달·서체 승계·툴바 스플릿 버튼, 7-에이전트 병렬 동작 보존 리팩토링",
      "웹 서비스(PDF Editor Live)와 에디터 코어를 공유 — Platform 인터페이스로 양방향 동기화",
    ],
  },
  {
    slug: "file-converter",
    name: "파일 변환기 — 오프라인 이미지·PDF 변환기",
    oneLiner: "AI 배경 제거까지 완전 오프라인으로 처리하는 파일 변환기 (Electron)",
    period: "2026.07 ~",
    company: "개인 프로젝트",
    domain: "데스크톱",
    description: [
      "이미지·PDF를 오프라인에서 상호 변환하는 데스크톱 앱입니다. 온라인 변환 사이트에 파일을 올리는 것 자체가 내 데이터를 제3자에게 주는 일이고, 무료처럼 쓰다 어느 순간 결제를 요구받는 것이 싫어 직접 만들었습니다 — AI 배경 제거를 포함한 모든 처리가 네트워크 없이 로컬에서 끝납니다.",
      "HEIC·TIFF·ICO·SVG까지 입출력을 지원하고 리사이즈·회전·자르기·워터마크·undo/redo, PDF 병합·분할·페이지 도구까지 갖춘 변환 특화 앱입니다. PDF 편집 기능이 커지자 pdf-editor를 별도 프로젝트로 분리했습니다.",
    ],
    stack: [
      "Electron 33",
      "React 18",
      "TypeScript",
      "MUI 6",
      "ONNX (ISNet)",
      "onnxruntime-web",
      "pdf.js",
      "pdf-lib",
      "Playwright",
    ],
    highlights: [
      "AI 배경 제거 완전 오프라인 — ISNet 모델·wasm 354MB를 인스톨러에 번들",
      "입출력 11종 포맷 상호 변환 (HEIC·TIFF·ICO·SVG 포함)",
      "PDF 병합·분할·회전·삭제·순서변경 문서 도구",
      "Playwright E2E 24종 + 난독화 릴리스 파이프라인",
    ],
  },
  {
    slug: "dicom-studio",
    name: "DICOM Studio — 검사결과 DICOM 변환·PACS 게이트웨이",
    oneLiner: "장비 출력물(PDF·이미지)을 DICOM 3.0으로 변환해 PACS 전송까지 (Electron)",
    period: "2026.07 ~",
    company: "개인 프로젝트",
    domain: "의료",
    description: [
      "병원 검사장비(혈관검사·심전도 등) 상당수는 결과를 DICOM이 아닌 PDF·JPG 같은 일반 파일로 출력합니다. 이를 PACS에 넣으려면 환자·검사정보를 결합해 DICOM으로 변환하는 게이트웨이가 필요한데, 기존 상용 도구는 특정 장비·벤더에 종속되고 라이선스 인증이 번거로워 범용 도구를 직접 만들었습니다.",
      "파일 열기(매직 바이트 형식 감지) → 뷰어 정리(회전·반전·순서) → 환자·검사정보 입력(워크리스트 연동 자동 입력) → DICOM 3.0 변환 → 로컬 DB 보관·검색 → PACS 전송(C-STORE)까지 검사실 워크플로우 전체를 담았고, 전송을 제외한 모든 기능이 오프라인 단독 구동입니다.",
    ],
    stack: [
      "Electron 39",
      "React 19",
      "TypeScript",
      "Tailwind CSS 4",
      "dcmjs",
      "dcmjs-dimse",
      "pdf.js",
      "sql.js",
      "Playwright",
    ],
    highlights: [
      "DICOM 3.0 Secondary Capture 변환 — 다중 이미지 = 한 Study 한 Series",
      "DICOM 네트워크 3종 — C-STORE 전송·C-FIND 워크리스트·C-ECHO 연결 테스트",
      "sql.js(WASM SQLite) 내장 DB — 네이티브 모듈 0개로 크로스빌드 함정 제거",
      "로컬/서버 전환 워크리스트 연결 프로필 + SaveDB 시 일치 처방 자동 완료",
    ],
  },
  {
    slug: "sh-dicom-studio",
    name: "sh DICOM Studio — DICOM 게이트웨이 C# 재구현",
    oneLiner: "직접 만든 dicom-studio를 C#·ASP.NET Core·Oracle·도커로 다시 만든 학습 마이그레이션",
    period: "2026.08 ~",
    company: "개인 프로젝트",
    domain: "의료",
    description: [
      "sh-ip-scanner로 C# 문법을 익힌 다음 단계로, 실무 규모의 앱을 C# 생태계 전체(데스크톱 MVVM → 웹 API → Oracle → 컨테이너 배포)로 관통하며 배우기 위한 마이그레이션 프로젝트입니다. 교재는 직접 만들었던 dicom-studio(Electron) — 기능 명세를 이미 알고 있으니 언어·스택 학습에만 집중할 수 있습니다.",
      "1차 오프라인 완결(이미지 → DICOM 변환·뷰어·SQLite 검색)에서 시작해 2차 서버(ASP.NET Core 8 + Oracle, 도커), 3차 PACS 전송(C-ECHO/C-STORE, Orthanc 동봉), 4차 Worklist(예약 접수)까지 완주해 v1.0으로 승격했습니다. 학습 과정은 12강짜리 'JAVA 개발자를 위한 C# & ASP.NET 실전 입문' 교재 PDF로 정리해 릴리스에 함께 공개했습니다.",
    ],
    stack: [
      "C# 12",
      ".NET 8",
      "Avalonia UI 11",
      "ASP.NET Core 8",
      "Oracle 23ai",
      "Docker Compose",
      "fo-dicom",
      "Orthanc",
      "SQLite",
      "JWT",
      "xUnit",
    ],
    highlights: [
      "같은 도메인을 두 스택으로 — Electron/dcmjs 구현과 C#/fo-dicom 구현을 1:1 비교하며 학습",
      "ASP.NET Core 8 Minimal API + Oracle 23ai Free를 docker compose로 — JWT 로그인·검사 메타 동기화·예약(Worklist) API",
      "PACS 전송 E2E — 동봉한 Orthanc 컨테이너로 C-ECHO → C-STORE 실전송 검증",
      "학습 산출물을 교재로 — 앱의 실제 코드로 쓴 C#/ASP.NET 12강 PDF를 릴리스에 공개",
    ],
  },
  {
    slug: "sh-ip-scanner",
    name: "sh IP Scanner — LAN IP·PC명 스캐너",
    oneLiner: "C#/.NET을 제대로 배우려고 만든 254칸 바둑판 LAN 스캐너 (Avalonia)",
    period: "2026.08 ~",
    company: "개인 프로젝트",
    domain: "데스크톱",
    description: [
      "TypeScript·React·Electron 위주로 써온 스택에서 벗어나 새 언어 하나를 제대로 익히려고 시작한 학습 프로젝트입니다. 교재로는 실제로 쓰고 있던 포터블 툴 faIpScanner(Delphi로 만들어진 블랙박스 exe)를 골라, 바이너리 정적 분석으로 동작 원리(멀티스레드 스윕·SendARP·역DNS)를 밝힌 뒤 C#/.NET 8/Avalonia로 재현했습니다.",
      "서브넷을 병렬 핑 스윕해 254칸 바둑판을 실시간으로 색칠하고(흰=미확인·주황=사용 중·연두=사용 가능), 사용 중인 IP의 PC명을 NetBIOS(UDP 137, 한글 CP949 디코드)와 역DNS로 조회합니다. 결과물로도 실제 전산 자산 파악에 쓰고 있으며, 스캔 대역은 여러 개를 드롭다운으로 오가며 관리합니다.",
    ],
    stack: [
      "C# 12",
      ".NET 8",
      "Avalonia UI 11",
      "MVVM",
      "CommunityToolkit.Mvvm",
      "async/await",
      "xUnit",
      "Inno Setup",
    ],
    highlights: [
      "원본 exe 정적 분석으로 동작 원리를 밝히고 C#으로 재현 — 스레드 254개 대신 async/await + SemaphoreSlim",
      "NetBIOS NBSTAT(UDP 137) 직접 구현 — 한글 PC명을 CP949로 디코드, 실패 시 역DNS 폴백",
      "Avalonia 헤드리스 렌더러로 GUI 없는 WSL에서 UI를 PNG로 확인하는 캡처 하네스",
      "자체포함 게시 + Inno Setup 인스톨러 — WSL에서 wine으로 Setup.exe까지 굽는 파이프라인",
    ],
  },
  {
    slug: "cm-groupware",
    name: "CM병원 그룹웨어",
    oneLiner: "13개 업무 도메인을 DDD로 모듈화한 병원 통합 업무 시스템",
    period: "2026.05 ~",
    company: "씨엠병원",
    domain: "의료",
    featured: true,
    description: [
      "전자결재·근태/연차·게시판·사내 메일/메신저·문서 발급·직무평가·프로젝트 칸반을 단일 플랫폼으로 통합한 사내 그룹웨어. Laravel 12 DDD 구조로 13개 도메인을 분리하고 프로덕션 운영 중입니다.",
      "AI(Claude Code) 페어 프로그래밍을 핵심 개발 방식으로 채택하고, hooks·슬래시 커맨드·프로젝트 전용 MCP 서버로 AI 행동을 통제·검증하는 자동화 레이어를 직접 설계했습니다.",
    ],
    stack: [
      "Laravel 12",
      "PHP 8.4",
      "Filament 5",
      "Livewire 4",
      "Tailwind CSS 4",
      "MySQL",
      "Docker",
      "Mattermost",
      "Mailcow",
      "Claude Code",
      "MCP",
    ],
    highlights: [
      "다형성(Polymorphic) 전자결재 엔진 — 이종 문서를 단일 결재 흐름으로 처리",
      "사용자 생성 시 메일/메신저 계정 자동 프로비저닝 (이벤트-리스너)",
      "TailAdmin 디자인 시스템을 Blade로 이식, 다크모드 지원",
      "ADR 7건으로 아키텍처 의사결정 영구 기록",
    ],
  },
  {
    slug: "meeting-todo-mcp",
    name: "AI 회의록 → TODO 자동 등록 (LLM + MCP)",
    oneLiner: "그룹웨어를 MCP 서버로 노출해 LLM이 직접 칸반 카드를 등록",
    period: "2026.05",
    company: "씨엠병원",
    domain: "AI",
    featured: true,
    description: [
      "전사된 회의록을 LLM으로 요약·분석하고 회의 중 도출된 Action Item을 부서별 칸반 보드에 자동 등록하는 AI 연동 시스템입니다.",
      "그룹웨어를 HTTP/SSE 기반 MCP 서버로 노출하고 AI 서버가 MCP 클라이언트로 연결되어, LLM이 도구 호출을 자율 판단하도록 tool description과 JSON Schema를 설계했습니다.",
    ],
    stack: ["laravel/mcp", "Claude Code CLI", "Python", "HTTP/SSE", "Bearer + HMAC"],
    highlights: [
      "회의록 자동화 도구 11종 구현 (조회 + 생성)",
      "모든 생성 도구 멱등(idempotent) 설계 — 재시도·중복 호출 안전",
      "부서 책임자 자동 귀속으로 활동 로그 추적성 확보",
      "REST API·DB 직접 접근 대비 MCP 채택 근거를 ADR로 문서화",
    ],
  },
  {
    slug: "voice-server",
    name: "Voice Server — AI 회의록 전사 파이프라인",
    oneLiner: "화자분리 → STT → 요약 → 그룹웨어 전송 4단계 AI 파이프라인",
    period: "2026.05 ~",
    company: "씨엠병원",
    domain: "AI",
    description: [
      "음성 파일 업로드부터 화자분리, STT, LLM 요약, 그룹웨어 전송까지 수행하는 회의록 전사 서버입니다. GPU 처리를 원격 API(WhisperX·Claude 래퍼)로 위임하는 thin orchestrator로 아키텍처를 진화시켰고, 그 과정을 ADR 6건으로 추적했습니다.",
      "회의록 본문은 사용자 패스워드 기반 PBKDF2 → AES-256-GCM으로 암호화되어 서버조차 복호화할 수 없는 zero-knowledge에 가까운 설계입니다.",
    ],
    stack: [
      "Python",
      "FastAPI",
      "WhisperX",
      "pyannote.audio",
      "SQLAlchemy",
      "MySQL",
      "AES-256-GCM",
      "HMAC-SHA256",
    ],
    highlights: [
      "queued → converting → diarizing → asr → summarizing → sending 상태머신",
      "단방향 outbound HTTPS Push + Bearer/HMAC 서명, 지수백오프 재시도",
      "PII 보호 — 음성 파일 즉시 삭제, 발화 텍스트 로그 미기록",
      "torch·CUDA 제거로 의존성 수 GB → 수백 MB 경량화",
    ],
  },
  {
    slug: "pt-schedule",
    name: "PT Schedule — 물리치료실 스케줄 관리",
    oneLiner: "치료사별 시간표·월간 통계·검색을 제공하는 사내 운영 도구",
    period: "2026.06 ~",
    company: "씨엠병원",
    domain: "의료",
    description: [
      "물리치료실 치료사별 일일 시간표(08:00~18:00)에 환자를 배정하고, 월간 캘린더 통계와 환자/치료사 검색을 제공하는 운영 시스템입니다. 관리자/치료사 이중 인증을 지원합니다.",
      "cm_groupware의 문서/하네스 표준(Skills 잠금·MCP 프로젝트 로컬 격리·커밋 컨벤션)을 경량 스택에 이식했습니다.",
    ],
    stack: [
      "FastAPI",
      "SQLModel",
      "React 18",
      "Vite",
      "Tailwind CSS",
      "TanStack Query",
      "MySQL",
      "Docker Compose",
    ],
    highlights: [
      "schedule·therapist·auth·calendar·search 5개 기능 도메인",
      "치료사별 색상 시간표 + 월간 환자 수 통계",
      "skills-lock.json 해시 고정으로 AI 도구 재현성 확보",
    ],
  },
  {
    slug: "dpp-performance",
    name: "EV 배터리 DPP 블록체인 성능 시험",
    oneLiner: "ICON 네트워크 READ 3,000 TPS / WRITE 1,000 TPS 정량 검증",
    period: "2025.10 ~ 2026.01",
    company: "파라메타",
    domain: "블록체인",
    description: [
      "KISA·부산광역시 주관 블록체인 기반 전기차 배터리 이력관리·DPP 인증 사업에서 성능·부하 시험을 담당했습니다.",
      "TTA 공식 성능시험에 대응해 nGrinder 부하 시나리오를 설계하고 인증용 성능시험 보고서를 작성했습니다.",
    ],
    stack: ["nGrinder", "Groovy", "ICON SDK", "NCP", "MySQL"],
    highlights: [
      "코인 네트워크 READ 3,000 / WRITE 1,000 TPS 측정·검증",
      "토큰 네트워크 READ 1,000 / WRITE 500 TPS 측정·검증",
      "단계별 부하 인가로 네트워크별 한계 처리량(임계점) 도출",
      "LFT2(PBFT 기반 BFT) 합의 구조 기반 Mainnet/Testnet 성능 편차 분석",
    ],
  },
  {
    slug: "athometrip-commerce",
    name: "앳홈트립 주문·결제 서비스",
    oneLiner: "워드프레스 레거시를 Stripe 연동 자체 플랫폼으로 이관",
    period: "2025.02 ~ 2025.08",
    company: "앳홈트립",
    domain: "커머스",
    description: [
      "워드프레스 운영 한계와 사업 확장 수요를 해소하기 위한 자체 주문·결제 플랫폼입니다. Stripe 연동 결제 구조를 설계하고 RabbitMQ 비동기 메시징으로 주문 후처리를 분리했습니다.",
    ],
    stack: [
      "Java",
      "Spring Boot",
      "MyBatis",
      "RabbitMQ",
      "Stripe",
      "React.js",
      "MariaDB",
      "AWS EC2",
    ],
    highlights: [
      "Stripe 주문/결제 구조 설계·개발",
      "RabbitMQ로 알림·정산 후처리 분리, 결제 응답 지연 최소화",
      "상품 주문 ↔ 가이드/협력사 매칭 로직 구현",
    ],
  },
  {
    slug: "gaia-backoffice",
    name: "GAIA 백오피스",
    oneLiner: "NestJS MSA 6개 서비스를 단독 설계한 여행 상품 운영 시스템",
    period: "2024.05 ~ 2025.01",
    company: "앳홈트립",
    domain: "커머스",
    featured: true,
    description: [
      "내부 CS와 외부 협력사(가이드·발권·셔틀)가 주문 관리와 일정 리마인드를 수행하는 어드민 시스템입니다. 주문/발권/리마인드 도메인을 분리한 NestJS MSA 6개 서비스를 단독 설계·개발했습니다.",
      "WooCommerce Webhook 이벤트를 WebSocket으로 CS 화면에 실시간 반영해 수기 확인 단계를 제거했습니다.",
    ],
    stack: [
      "TypeScript",
      "NestJS (MSA)",
      "Next.js",
      "TanStack Query/Table",
      "Auth.js",
      "WebSocket",
      "MariaDB",
      "AWS",
    ],
    highlights: [
      "6개 마이크로서비스 단독 설계·개발",
      "Webhook → WebSocket 실시간 주문 반영",
      "대용량 주문 데이터 어드민 UI (TanStack)",
      "협력사 역할별 접근 제어",
    ],
  },
  {
    slug: "fireblocks-custody",
    name: "디지털 자산 커스터디 서명·전송 시스템",
    oneLiner: "Fireblocks MPC 기반 다중 승인 전송 워크플로",
    period: "2023.06 ~ 2023.09",
    company: "위메이드",
    domain: "블록체인",
    description: [
      "사내 권한자(editor·approver·signer)가 역할에 따라 원장을 생성·서명·전송하는 기관용 디지털 자산 관리 시스템입니다. Fireblocks MPC 커스터디 SDK를 연동해 다중 승인 기반 전송 워크플로를 구현했습니다.",
    ],
    stack: [
      "Node.js (Express)",
      "React.js",
      "Redux",
      "Fireblocks SDK",
      "JWT",
      "Redis",
      "Azure",
    ],
    highlights: [
      "Fireblocks SDK 지갑 생성·서명·전송 API",
      "RBAC + 다중 승인(Multi-approval) 권한 처리",
      "기관용 자산/트랜잭션 로그 조회",
    ],
  },
  {
    slug: "wcms",
    name: "WCMS — 온체인 원장 데이터 대시보드",
    oneLiner: "web3.js 스케줄러 기반 온체인 데이터 수집 자동화",
    period: "2023.05 ~ 2023.11",
    company: "위메이드",
    domain: "블록체인",
    description: [
      "재단 지갑 및 암호화폐의 온체인 원장 데이터를 수집·관리하는 재무 대시보드입니다. 외부 스캐너 데이터의 정합성 한계를 보완하기 위해 원장 데이터를 직접 수집·적재하는 파이프라인을 구축했습니다.",
    ],
    stack: [
      "Java",
      "Spring Batch",
      "Node.js",
      "web3.js",
      "TypeScript",
      "MySQL",
      "Azure SQL",
    ],
    highlights: [
      "일별 블록 범위 수집 + 재단 지갑별 토큰 집계",
      "FT/NFT 분류 및 tx-log 수집, UTC·KST 동시 산출",
      "월별 CSV 자동 덤프로 감사·정산 추적성 확보",
    ],
  },
  {
    slug: "kr-esg-nft",
    name: "한국렌탈 ESG NFT 대시보드",
    oneLiner: "Klaytn Mainnet에 KIP-17 NFT 발행·증여 + ERP 연동",
    period: "2022.09 ~ 2023.01",
    company: "드림시큐리티",
    domain: "블록체인",
    description: [
      "ESG 캠페인(연말 나무 심기)의 식수 좌표를 메타데이터에 담아 NFT로 발행·증여하고, 한국렌탈 ERP에 대시보드로 연동한 서비스입니다.",
    ],
    stack: [
      "Solidity",
      "Klaytn",
      "caver-js",
      "Truffle",
      "IPFS (Infura)",
      "Node.js",
      "Next.js",
      "Recoil",
    ],
    highlights: [
      "KIP-17(ERC-721 호환) 컨트랙트 구현, Klaytn Mainnet 배포",
      "IPFS 메타데이터 저장 + 온체인 발행·증여 트랜잭션 처리",
      "NFT 발행/증여 현황 대시보드 + ERP 연동",
    ],
  },
  {
    slug: "magicdid",
    name: "MagicDID 운영 백오피스",
    oneLiner: "HyperLedger Fabric DID 생애주기 모니터링",
    period: "2022.06 ~ 2022.08",
    company: "드림시큐리티",
    domain: "블록체인",
    description: [
      "HyperLedger Fabric 기반 분산신원증명(DID) 서비스의 발급·갱신·폐기 현황을 모니터링하는 운영 백오피스입니다. DID 생애주기 지표를 시각화해 운영 가시성을 확보했습니다.",
    ],
    stack: ["React.js", "Redux-Saga", "ECharts", "MUI", "JWT", "On-Premise"],
    highlights: [
      "DID 발급/갱신/폐기 지표 ECharts 시각화",
      "LG 모나체인 백오피스 벤치마킹 UI 설계",
    ],
  },

];
