...
export const experiences: Experience[] = [
  {
    ...
    projects: [
      ...
      },
      {
        ...
        period: "26.05 ~",
        description: [
          "부서장 회의 음성 파일을 업로드하면 화자분리 → STT → LLM 요약 → 그룹웨어 전송까지 자동으로 처리하는 회의록 전사 서버",
          "GPU 1대(12GB VRAM) 제약에서 출발해 처리 단계를 원격 API로 위임하는 thin orchestrator 구조로 아키텍처를 진화시키고, 그 과정을 ADR 6건으로 추적",
        ],
        stack: [
          "Python",
          "FastAPI",
        ...
        tasks: [
          "queued → converting → diarizing → asr → summarizing → sending → done 상태머신을 설계하고 단계별 처리 시각을 DB에 기록해 병목을 추적",
          "로컬 pyannote/faster-whisper GPU 처리 → 원격 WhisperX API 위임 → 요약도 원격 Claude 래퍼 API 위임으로 전환, torch·CUDA 의존성을 제거해 수 GB → 수백 MB로 경량화",
          "회의록 본문을 업로드 시 입력한 패스워드로 PBKDF2 → AES-256-GCM 암호화 — 패스워드를 어디에도 저장하지 않아 서버조차 본문을 복호화할 수 없는 구조로 설계",
          "그룹웨어와는 단방향 outbound HTTPS Push만 사용(NAT 뒤에서도 동작)하고 Bearer 토큰 + HMAC-SHA256 서명, 실패 시 지수백오프 5회 재시도 적용",
          "PII 보호 — 음성 파일은 잡 종료 즉시 삭제하고 발화 텍스트는 로그에 남기지 않음(job_id까지만 기록)",
          "asyncio.Queue 단일 워커 직렬 처리로 공유 GPU 자원을 보호하고, 그룹웨어와 DB·자격증명을 분리",
          "[AI] cm_groupware의 Harness Engineering 패턴 이식 — hooks 3종(env-guard·git-add-guard·ruff 자동 포맷), 슬래시 커맨드 3종(/review-pipeline·/review-security·/deploy-check)",
        ],
      },
      {
        ...
        period: "26.06 ~",
        description: [
          "물리치료실의 치료사별 일일 시간표(08:00~18:00) 환자 배정을 수기 관리에서 웹 시스템으로 전환",
          "관리자/치료사 이중 인증과 월간 캘린더 통계·환자/치료사 검색을 제공하는 운영 도구",
        ],
        stack: [
          "Python",
          "FastAPI",
        ...
        tasks: [
          "schedule·therapist·auth·calendar·search 5개 도메인을 기능별 라우터로 분리한 FastAPI 백엔드 설계·개발",
          "치료사별 시간표 그리드와 월간 캘린더 통계(치료사별 환자 수)를 React + TanStack Query로 구현, 치료사별 색상 구분 적용",
          "관리자/치료사 역할별 화면 분리 — 치료사는 개인 시간표, 관리자는 전체 운영 관리",
          "cm_groupware와 같은 서버에서 Docker Compose(edge 외부 네트워크 공유)로 운영하고 그룹웨어 사이드바에서 바로 진입하도록 연동",
        ],
      },
    ],
  },
